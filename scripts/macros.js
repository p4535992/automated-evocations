...
return {
  embedded: {
    Item: {
      'Arcane Sword': {
        'data.attackBonus':
          args[0].assignedActor?.data.data.attributes.spelldc -
          8 +
          args[0].assignedActor?.data.data.bonuses.msak.attack,
      },
    },
  },
};
  ...
  embedded: {
    Item: {
      Slash: {
        ...
        'data.damage.parts': [
          [
            `${1 + Math.floor((args[0].spellLevel - 2) / 2)}d8 + ${
              args[0].assignedActor?.data.data.abilities[args[0].assignedActor?.data.data.attributes.spellcasting]
                ?.mod || ''
            }`,
            'force',
          ],
