...
Hooks.once("ready", () => {
    ...
    if (!game.modules.get("sequencer")?.active && game.user?.isGM) {
        Logger.error(
            `The '${CONSTANTS.MODULE_ID}' module requires to install and activate the 'sequencer' module.`,
            true,
        );
        return;
    }
    if (!game.modules.get("warpgate")?.active && game.user?.isGM) {
        Logger.error(
            `The '${CONSTANTS.MODULE_ID}' module requires to install and activate the 'warpgate' module.`,
            true,
        );
        return;
    }
    if (!game.modules.get("socketlib")?.active && game.user?.isGM) {
        Logger.error(
            `The '${CONSTANTS.MODULE_ID}' module requires to install and activate the 'socketlib' module.`,
            true,
        );
        return;
    }
