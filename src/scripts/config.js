    ...
    Hooks.on("renderTokenHUD", (app, html, data) => {
        if (game.settings.get(CONSTANTS.MODULE_ID, "hudEnable")) {
            ...
            if (html.find(".control-icon.automated-evocations-variant .fa-cat").length > 0) {
                html.find(".control-icon.automated-evocations-variant .fa-cat")[0].style.color =
                    `${settingHudColorButton}`;
                html.find(".control-icon.automated-evocations-variant .fa-cat")[0].style.textShadow =
                    `0 0 8px ${settingHudColorButton}`;
            }
        }
    });
